...
@section('content')
    ...
    <div class="container">
        <div class="row flex-nowrap">
            ...
            <a href="{{ route('states.create') }}" class="ml-auto btn btn-primary mb-2">
                    ...
                    <tbody>
                        @foreach ($states as $state)
                            <tr>
                                ...
                                <td>{{ $state->name }}</td>
                                <td>{{ $state->country->country_code .' - '. $state->country->name }}</td>
                                <td style="text-align:center" class="d-flex justify-content-center">
                                    <a href="{{ route('states.edit',['state'=> $state->id]) }}" class="btn btn-success btn-sm text-center"><i
                                    ...
                                    <form method="POST" action="{{ route('states.destroy', $state->id) }}">
                                        ...
                                        onclick="deleteButtonClick(event, this, this.parentElement, '{{ $state->name }}')"
                                        data-confirmation-window-title="{{__('Confirm state deletion from database')}}"
                                        data-confirmation-window-text="{{__('Do you want to delete ').$state->name}}"
