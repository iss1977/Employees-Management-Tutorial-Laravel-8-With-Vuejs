                                ...
                                <div class="col-md-6">
                                    ...
                                    <select id='select_country' name='country_id' class="custom-select" required
                                        ...
                                        @foreach ($countries as $country)
                                            <option class="remove-on-update" value='{{ $country->id }}'
                                    ...
                                    <select id='select-state' name='state_id' class="custom-select" required>
                                        ...
                                        @forelse ($states as $state)
                                            ...
                                            @if ($state->id == $currentState->id)
                                                selected
                                            @endif class="remove-on-update">
                                                {{ $state->name }}
                                            </option>
                                        @empty
                                            <option value='' required selected>-- No states in current country --</option>
                                        @endforelse
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-md-6">
                                    <input id="name" type="text" class="form-control @error('name') is-invalid @enderror"
                                        name="name" value="{{ old('name',$city->name) }}" required autocomplete="city_name" autofocus>
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (len > 0) {
                        ...
                        var option = `<option value='' required selected class="remove-on-update">-- No states in current country --</option>`
                        $("#select-state").append(option);
                    }
